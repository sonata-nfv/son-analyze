...
docker run -i --rm=true  --env="${EXTRA_ENV}" -v "/var/run/docker.sock:/var/run/docker.sock" 'son-analyze-test' son-analyze bootstrap
...
docker run -i --rm=true  --env="${EXTRA_ENV}" -v "/var/run/docker.sock:/var/run/docker.sock" --workdir '/var/tmp/son.analyze' 'son-analyze:latest' /usr/bin/R -e 'devtools::check()'
